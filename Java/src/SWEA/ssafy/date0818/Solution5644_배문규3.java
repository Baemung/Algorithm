...
class BC2{
	...
	BC2(int x, int y, int d, int charge, int serial){
		this.x = x;
		this.y = y;
		this.d = d;
		this.charge = charge;
...
public class Solution5644_배문규3 {
	...
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		//for(int tc = 1; tc <= T; tc++) {
			...
			for(int i = 0; i < N; i++) {
				st = new StringTokenizer(br.readLine());
				bc[i] = new BC2(Integer.parseInt(st.nextToken())-1, Integer.parseInt(st.nextToken())-1, Integer.parseInt(st.nextToken()), Integer.parseInt(st.nextToken()), i);
			}
	...
	public static void setBC() {
		...
		for(BC2 b : bc) {
			...
			if(Math.abs(Aidx[0]-b.y) + Math.abs(Aidx[1]-b.x) <= b.d) Alist[0] = b.charge;
			if(Math.abs(Bidx[0]-b.y) + Math.abs(Bidx[1]-b.x) <= b.d) Blist[0] = b.charge;
		}
