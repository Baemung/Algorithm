...
public class SEOUL06_BAEMUNGYU {

	// 닉네임을 사용자에 맞게 변경해 주세요.
	static final String NICKNAME = "SEOUL06_BAEMUNGYU";
	...
	public static void main(String[] args) {
		...
		try {
			...
			while (socket != null) {
				...
				// 현재 당구대위에 남아있는 당구공 찾음
				int x = -1; // 당구대 위에 있는 당구공 번호 변수
				for(int i = 1; i < NUMBER_OF_BALLS; i++) {
					if(!(balls[i][0] == -1 && balls[i][1] == -1)) { // 공이있다면
						x = i; // 변수 x에 해당 공번호 저장
						break;
					}
				}
				...
				{
					radian = Math.atan(width / height);
					angle = (float) (((180.0 / Math.PI) * radian)*-1 + 360);
				}
